...
* **Observability by Default & Anti-Monolith Modularity ([AGENTS.md Principle 3](../../AGENTS.md)):** Provide comprehensive tracing and correlated structured logging while separating concerns across modular components (`ArgumentSanitizer`, `ModelRequestContext`, `ToolManager`).
...
### Option 3: Standardized OTel GenAI Model Spans, Enriched Tool Spans via `ToolManager` with `ArgumentSanitizer`, Sampled Thought Logging, and Enriched Span Events (Selected)
...
| **B2: Stable vs Internal** | Explicitly Delineated | **Stable Contract:** OTel span attribute keys (`gen_ai.*`, `agent.*`), span event naming syntax (`agent.tool.call: <tool>`), and structured log keys. **Internal Implementation:** `ArgumentSanitizer`, `ModelRequestContext`, `ToolManager` helper methods. |
